...
exports.create = function(options, callback) {
  ...
  sig.keyInfoProvider = {
    getKeyInfo: function () {
      options.signatureSettings = options.signatureSettings || {};
      var prefix = options.signatureSettings.prefix ? options.signatureSettings.prefix + ':' : '';
      return util.format("<%sX509Data><%sX509Certificate>%s</%sX509Certificate></%sX509Data>", prefix, prefix, cert, prefix, prefix);
    }
  };
  ...
  var now = moment.utc();
  if (options.clockSkew) now.subtract(options.clockSkew,'seconds');
  ...
  options.signatureSettings = options.signatureSettings || {};
  options.signatureSettings.location = options.signatureSettings.location || {};
  options.signatureSettings.location.reference = options.signatureSettings.location.reference || "//*[local-name(.)='Issuer']";
  options.signatureSettings.location.action = options.signatureSettings.location.action || 'after';
  var signed;
  try {
    sig.computeSignature(token, options.signatureSettings);
